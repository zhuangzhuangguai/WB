...
parameter.clustering <- pam(dftemp[, -1], k = ngroups, cluster.only = F)
slope.clustering <- pam(dist(na.omit(slope.df[, -1])), k = ngroups, cluster.only = F)

parameter.groups <- parameter.clustering$clustering
slope.groups <- slope.clustering$clustering

parameter.plotdata <- cbind(dftemp, parameter.groups)
slope.plotdata <- cbind(filter(dftemp, Date %in% slope.df$Date), slope.groups)
...
parameter.pca <- prcomp(na.omit(dftemp[, -1]), center = T, scale. = T)
slope.pca <- prcomp(na.omit(slope.df[, -1]), center = T, scale. = T)

parameter.tsne <- Rtsne(as.matrix(na.omit(dftemp[, -1])), check_duplicates = FALSE, pca = FALSE, perplexity=30, theta=0.5, dims=2)
slope.tsne <- Rtsne(as.matrix(na.omit(slope.df[, -1])), check_duplicates = FALSE, pca = FALSE, perplexity=30, theta=0.5, dims=2)
